  ...
  return (
    <div style={{ background: 'var(--paper)', minHeight: '100vh', fontFamily: 'var(--mono)' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border-solid)', padding: '12px 28px', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--paper)', position: 'sticky', top: 0, zIndex: 10 }}>
        <a href="/about" style={{ fontSize: 10, color: 'var(--muted)', textDecoration: 'none', padding: '4px 8px', border: '1px solid var(--border-solid)', letterSpacing: '.04em' }}>
          ← About
        </a>
        ...
        <span style={{ fontSize: 10, color: 'var(--muted)', marginLeft: 'auto', letterSpacing: '.06em' }}>▶ FORWARD · B·C·C</span>
      </div>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '52px 28px 80px' }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '.12em', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 12 }}>
            Ask Ozzy
          </div>
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 38, color: 'var(--ink)', lineHeight: 1.2, margin: '0 0 14px', fontWeight: 400 }}>
            What do you want to know about Birmingham?
          </h1>
          <p style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.8, maxWidth: 560, margin: 0 }}>
            Pick a question below. Ozzy gives you a straight answer backed by the city&apos;s public data.
          </p>
          <div style={{ marginTop: 12, fontSize: 10, color: 'var(--muted2)', fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {QUESTIONS.map((item, i) => (
            <div key={i} style={{ borderTop: '1px solid var(--border-solid)', ...(i === QUESTIONS.length - 1 ? { borderBottom: '1px solid var(--border-solid)' } : {}) }}>
              ...
              >
                <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--ink)', lineHeight: 1.4 }}>
                  {item.q}
                </span>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 14, color: 'var(--muted)', flexShrink: 0 }}>
                  {active === i ? '−' : '+'}
                </span>
              </button>
              ...
              {active === i && (
                <div style={{ padding: '0 0 24px 0' }}>
                  <div style={{ background: 'var(--ink)', padding: '20px 22px', marginBottom: 12 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--r1)', letterSpacing: '.1em', marginBottom: 10 }}>
                      OZZY · BIRMINGHAM INTELLIGENCE
                    </div>
                    {item.a.split('\n\n').map((para, pi) => (
                      <p key={pi} style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--border-solid)', lineHeight: 1.8, margin: pi < item.a.split('\n\n').length - 1 ? '0 0 12px' : 0 }}>
                        {para}
                      </p>
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {item.links.map(link => (
                      <a key={link.href} href={link.href} style={{ fontFamily: 'var(--mono)', fontSize: 10, padding: '6px 14px', border: '1px solid var(--ink)', color: 'var(--ink)', textDecoration: 'none', letterSpacing: '.04em' }}>
                        {link.label}
                      </a>
                    ))}
                  </div>
        ...
        {/* Footer note */}
        <div style={{ marginTop: 52, paddingTop: 28, borderTop: '1px solid var(--border-solid)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ fontSize: 10, color: 'var(--muted2)' }}>
            Data: NOMIS · IMD 2025 · Census 2021 · WMP · City Observatory
          </div>
          <a href="/about" style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', textDecoration: 'none', letterSpacing: '.04em' }}>
            About Ozzy →
          </a>
        </div>
      </div>
